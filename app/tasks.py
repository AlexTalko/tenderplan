...
class FetchPageTask(Task):
    name = "fetch_page_task"  # Имя задачи
...
class ParseXmlTask(Task):
    name = "parse_xml_task"  # Имя задачи
    ...
    def run(self, print_form_url):
        try:
            ...
            # logger.info(f"Структура XML: {xml_dict}")  # Логируем структуру XML

            # Ищем значение publishDTInEIS во всём XML
